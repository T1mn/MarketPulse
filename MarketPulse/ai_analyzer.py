...
def analyze_news_in_batch(articles):
    ...
    try:
        ...
        ---
        ### 特别指令：顶级新闻社VIP分析标准
        对于来源（source）为以下列表中的顶级新闻社的新闻，你的分析必须遵循更高、更严格的标准：
        - **顶级新闻社列表**: {list(config.TOP_TIER_NEWS_SOURCES)}
        - **市场影响分析 (`market_impact.market`)**: 必须给出最具体的影响范围，例如"美股半导体板块"、"离岸人民币汇率"、"WTI原油期货"。严禁使用"综合市场"、"全球股市"等模糊词汇。
        - **来源可信度 (`source_confidence`)**: 必须给出95%-100%之间的一个具体数值。
        ---

        重要提示:
        1. 最终输出必须是一个完整的JSON数组，以 `[` 开始，以 `]` 结束。
        2. 如果某项分析不明确或无法确定（非顶级来源新闻），请使用 "未知" 作为值。
