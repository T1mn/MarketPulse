...
class RSSNewsFetcher:
            ...
            if hasattr(entry, 'category') and entry.category:
                ...
                if isinstance(entry.category, list):
                    for cat in entry.category:
                        if isinstance(cat, dict) and cat.get('domain') == 'stock-symbol':
                            if cat.get('content'):
    ...
    def fetch_rss_news(self, rss_url: str, source_name: str, max_items: int = 10) -> List[Dict[str, Any]]:
        """
        从指定的RSS源获取新闻
    ...
    def fetch_bloomberg_news(self) -> List[Dict[str, Any]]:
        ...
        for source_name, rss_url in config.BLOOMBERG_RSS_SOURCES.items():
            try:
                ...
                news = self.fetch_rss_news(rss_url, f"Bloomberg-{source_name}", config.MAX_NEWS_PER_CATEGORY)
