...
def send_bark_notification(analysis_data, article_url, article_source=None, related_symbols=None, article_datetime=None):
    """
    ...
    try:
        ...
        if success_count > 0:
            print(f"Bark通知发送成功！成功发送到 {success_count}/{len(config.BARK_KEYS)} 个设备")
        else:
            print("所有设备通知发送失败！")
